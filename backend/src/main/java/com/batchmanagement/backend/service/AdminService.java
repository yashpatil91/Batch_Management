...
public class AdminService {
	...
	public AdminService(UserRepository userRepository, BatchRepository batchRepository,
			ModuleRepository moduleRepository, ModuleService moduleService, PasswordEncoder passwordEncoder) {
	...
	public List<UserResponse> getTrainers() {
		...
		return trainers.stream().map(trainer -> {
			int totalBatches = batchRepository.countByTrainerAndStatusNot(trainer, BatchStatus.COMPLETED);
	...
	public UserResponse updateTrainer(Long id, UserUpdateRequest request) {
		User trainer = userRepository.findById(id).filter(user -> user.getRole() == Role.TRAINER)
	...
	public List<UserResponse> getAdmins() {
		return userRepository.findByRole(Role.ADMIN).stream().map(UserMapper::toResponse).toList();
	}
	...
	public List<BatchWithModulesResponse> getAllBatchesWithModules() {
		return batchRepository.findAll().stream().map(batch -> {
			...
			response.setModules(moduleRepository.findByBatch(batch).stream().map(ModuleMapper::toResponse).toList());
			return response;
		}).toList();
	}
	...
	public BatchResponse assignBatch(AssignBatchRequest request) {
		...
		User trainer = userRepository.findById(request.getTrainerId()).filter(user -> user.getRole() == Role.TRAINER)
				.orElseThrow(() -> new ResourceNotFoundException("Trainer not found"));
		...
		try {
			emailService.sendBatchEmail(trainer.getEmail(), trainer.getName(), savedBatch.getDomainName(),
	...
	private void createModulesForBatch(Batch batch, List<CreateBatchRequest.ModuleRequest> modules,
			User defaultTrainer) {
		...
		for (CreateBatchRequest.ModuleRequest moduleRequest : modules) {
			...
			if (moduleRequest.getTrainerId() != null) {
				...
				System.out.println("[DEBUG][adminCreateBatch] validating module trainer schedule trainerId="
						+ trainer.getId() + " batchId=" + batch.getId() + " moduleName=" + moduleRequest.getName());
				moduleService.validateTrainerScheduleForBatch(trainer.getId(), batch.getId(), null);
				module.setTrainer(trainer);
			} else if (defaultTrainer != null) {
				System.out.println("[DEBUG][adminCreateBatch] validating default trainer schedule trainerId="
						+ defaultTrainer.getId() + " batchId=" + batch.getId() + " moduleName="
						+ moduleRequest.getName());
				moduleService.validateTrainerScheduleForBatch(defaultTrainer.getId(), batch.getId(), null);
	...
	public BatchResponse createBatch(CreateBatchRequest request) {
		...
		if (request.getTrainerId() != null) {
			trainer = userRepository.findById(request.getTrainerId()).filter(user -> user.getRole() == Role.TRAINER)
					.orElseThrow(() -> new ResourceNotFoundException("Trainer not found"));
		...
		if (trainer != null) {
			...
			List<Batch> activeBatches = batchRepository.findByTrainerAndStatusNot(trainer, BatchStatus.COMPLETED);

			for (Batch existing : activeBatches) {

				boolean dateOverlap = !request.getStartDate().isAfter(existing.getEndDate())
				...
				if (isTimeConflict(request.getTime(), existing.getTime())) {
					throw new BadRequestException("Trainer already has another active batch during this time.");
				}
			}
		}
		...
		// LAB CONFLICT CHECK
		// =========================
		List<Batch> labBatches = batchRepository.findByLabNoAndStatusNot(request.getLabNo(), BatchStatus.COMPLETED);
		...
		for (Batch existing : labBatches) {
			...
			if (isTimeConflict(request.getTime(), existing.getTime())) {
				throw new BadRequestException("Lab already occupied during this time.");
			}
		}
